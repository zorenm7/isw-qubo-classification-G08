Here is the code for the `src/qubo_project/preprocessing.py` module, strictly adhering to the project specifications outlined in your document.

### **Implementation Details**

* **Z-Score Normalization:** Implemented using `scikit-learn`'s `StandardScaler` to ensure features are transformed to have a mean of 0 and a standard deviation of 1.
* **Feature Filtration:** The script filters out features where the count of non-zero and non-null values falls below the `minPercValid` threshold.
* **JSON Reporting:** Generates the exact JSON schema requested, including timing statistics for both I/O and processing steps.
* **CLI Interface:** Uses `argparse` to handle the required command-line arguments (`--input`, `--target`, `--out-data`, `--out-json`, `--min-perc-valid`).

---

### **Source Code: `preprocessing.py`**

Create the `src/qubo_project/` directory if it doesn't exist, and place the following code in `preprocessing.py`:
...
Make sure that `scikit-learn` and `pandas` are included in your `requirements.txt` as they are explicitly used here.